...
Raku could have either cooperative threads or preemptive threads.
...
The interpreter has to do asynchronous I/O. But Raku may support
...
requirements. If Raku gets preemptive threads, expect race conditions
...
If Raku implements preemptive threads, then the interpreter must lock
...
One of the goals of Raku is to get out from under the backwards
compatibility constraints that have boxed in Perl. Organizing the
threading model around the need to make Perl modules reentrant seems
...
pressed to see that the design of Raku should be driven by the need
to fix modules that are broken in Perl.
