...
Raku should be able to DWIMishly distinguish between strings which
...
Although there is no way for raku to be sure whether a user intends a
...
explicitly. Without these pragmas, Raku's behavior regarding packed
data would be like Perl's.
