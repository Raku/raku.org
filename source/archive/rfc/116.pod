...
The RFCs on multi-dimensional arrays for Raku address some of the
...
Raku could come up with a representation for such types as part
...
In any case, some of the raku guys have asked for a rundown of
...
In Raku it may be desirable to use C<@syntax> for all arrays - whether
...
to be determined which of these might belong in the core of raku, in order
to make PDL work more seamlessly, and which belong in the module.

Our take on this: raku should support a compact, numerical array type which is
...
raku RFCs on Ranges and Default Methods.
